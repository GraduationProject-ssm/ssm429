...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=27)
...
## 1.3 论文结构安排
编写在线画展系统相对应的论文，其实就是对开发完成的程序进行再次解读的过程。本论文从七个方面的内容讲解了开发的程序，具体内容如下：
...
# 3 系统分析
## 3.1系统可行性分析
需要使用大部分精力开发的在线画展系统为了充分降低开发风险，特意在开发之前进行可行性分析这个验证系统开发是否可行的步骤。本文就会从技术角度，经济角度，还有用户使用的程序的运行角度进行综合阐述。
### 3.1.1 技术可行性分析
...
## 3.3系统流程分析
管理员假如要操作系统提供的功能，那么管理员就要在系统的登录界面，填写管理员登录的账号信息，填写相应的密码信息，管理员需要保证这两者能够验证身份的账号以及密码信息的正确性，这样管理员就可以通过登录界面进入系统后台操作界面。图3.1就是开发的程序软件在线画展系统它的操作流程图。

![](/md/blog.001.png)

图3.1 系统操作流程图
### 3.3.1系统登录流程
在线画展系统的登录流程，针对的角色就是操作员的操作角色。在登录界面需要的必填信息就是账号信息，配上登录的密码信息就能登录在线画展系统，需要注意的就是必填的账号信息和登录密码信息，都需要进行验证，系统会判断账号还有填写的密码信息的正确性，只有这两者信息都正确了，就能成功登录在线画展系统了。系统登录流程图如下图。
...
# 4 系统设计
## 4.1系统概要设计
在线画展系统并没有使用C/S结构，而是基于网络浏览器的方式去访问服务器，进而获取需要的数据信息，这种依靠浏览器进行数据访问的模式就是现在用得比较广泛的适用于广域网并且没有网速限制要求的B/S结构，图4.1就是开发出来的程序工作原理图。
...
![打开新的 phpMyAdmin 窗口](/md/blog.011.png "打开新的 phpMyAdmin 窗口")
...
# 5 系统实现
## 5.1 作家信息管理
管理员可以查询，修改，删除作家信息。下图就是作家信息管理页面。

![](/md/blog.012.png)

图5.1 作家信息管理页面
## 5.2 作品分类管理
管理员可以对作品分类信息进行删除，查询和修改操作。下图就是作品分类管理页面。

![](/md/blog.013.png)

图5.2作品分类信息管理页面
## 5.3用户管理
管理员可以对用户信息进行查询，修改，删除操作。下图就是用户信息管理页面。

![](/md/blog.014.png)

图5.3 用户信息管理页面
## 5.4 首页信息
用户可以在首页访问在线画展系统方面信息，首页上面有导航栏，作家作品，用户订单，通知公告，留言反馈，个人中心，后台管理，点击不同的导航跳转到不同的显示页面。下图就是首页信息页面。
